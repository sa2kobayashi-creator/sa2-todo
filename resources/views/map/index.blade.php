    ...
    <main class="page-main map-main">
      ...
      <div class="map-layout">
        <aside class="map-sidebar panel">
          <h1 class="map-page-title">{{ __('マップ・ナビ') }}</h1>

          <section class="map-route-form-section">
            <h2 class="map-section-title">{{ __('ルート検索') }}</h2>
            <div class="map-route-form">
              <label>
                {{ __('出発地') }}
                <input type="text" id="map-origin" placeholder="{{ __('出発地を入力') }}" autocomplete="off" value="{{ $selectedRoute['originLabel'] ?? '' }}" />
              </label>
              <label>
                {{ __('目的地') }}
                <input type="text" id="map-destination" placeholder="{{ __('目的地を入力') }}" autocomplete="off" value="{{ $selectedRoute['destinationLabel'] ?? '' }}" />
              </label>
              <label>
                {{ __('移動手段') }}
              ...
              <div class="map-route-form-actions">
                <button type="button" class="button-link" id="map-show-route">{{ __('ルート表示') }}</button>
          ...
          <section class="map-save-section">
            <h2 class="map-section-title">{{ __('よく使うルート') }}</h2>
            <form method="post" action="/map" class="map-save-form" id="map-save-form">
              ...
              <label>
                {{ __('ルート名') }}
                <input type="text" name="name" id="map-save-name" placeholder="{{ __('例: 自宅→博多駅') }}" required />
              </label>
              ...
              <button type="submit" class="button-link secondary">{{ __('現在のルートを保存') }}</button>
            </form>

            @if(count($routes) === 0)
              <p class="hint">{{ __('保存されたルートはありません。') }}</p>
            @else
                ...
                @foreach($routes as $route)
                  ...
                  >
                    ...
                    <div class="map-route-card-actions">
                      <button type="button" class="text-btn map-load-route-btn">{{ __('表示') }}</button>
                      <a href="{{ $route['googleNavUrl'] }}" target="_blank" rel="noopener noreferrer" class="text-btn">{{ __('ナビ') }}</a>
                      <form method="post" action="/map/{{ $route['id'] }}/delete" class="map-inline-form" onsubmit='return confirm(@json(__('このルートを削除しますか？')))'>
                        @csrf
          ...
          <div id="map-canvas" class="map-canvas" aria-label="Google Map">
            @unless($hasGoogleMapsApiKey)
              <div class="map-canvas-placeholder">
                <p>{{ __('地図を表示するには Google Maps API キーが必要です。') }}</p>
                <p class="hint">{{ __('保存済みルートの「ナビ」ボタンで Google Maps アプリを開けます。') }}</p>
              </div>
            @endunless
          </div>
