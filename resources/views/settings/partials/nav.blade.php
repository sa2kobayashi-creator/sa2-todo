...
<div class="panel" id="footer-nav-settings">
  <h2>{{ __('表示メニュー管理') }}</h2>
  <p class="hint">{{ __('メニューごとに、スマートフォン下部とWeb（PC）ヘッダーの表示を選べます。未設定（既定）では、使えるメニューがスマートフォン下部にもすべて載ります。スマートフォン下部は1行:per件、最大:max件です。6件以上はアイコンを長押しすると全メニューが表示されます（6〜10件は2行、11件以上は3行）。全メニュー表示中にさらに長押しすると並べ替えできます。チェックを外した項目はスマートフォンの「⋮」に入ります（Webでは⋮は表示しません）。', ['per' => FooterNav::FOOTER_PER_ROW, 'max' => FooterNav::MAX_FOOTER]) }}</p>
