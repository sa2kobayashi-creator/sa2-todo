...
<div class="panel" id="footer-nav-settings">
  <h2>{{ __('表示メニュー管理') }}</h2>
  <p class="hint">{{ __('メニューごとに、スマートフォン下部とWeb（PC）ヘッダーの表示を選べます。スマートフォン下部は1行:per件、最大:max件です。6〜10件はバーを上にスワイプすると2行、11件以上は3行になります。スマホではアイコンを長押ししてドラッグすると並べ替えできます。選ばなかった項目はスマートフォンの「⋮」に入ります（Webでは⋮は表示しません）。', ['per' => FooterNav::FOOTER_PER_ROW, 'max' => FooterNav::MAX_FOOTER]) }}</p>
