<article
  ...
  tabindex="0"
  role="button"
  aria-label="{{ $account['name'] }} {{ __('の残高') }} {{ $formatMoney($account['balance'], $account['currency']) }}"
>
  <form method="post" action="/finance/accounts/{{ $account['id'] }}/overview" class="finance-balance-overview-remove-form" onclick="event.stopPropagation()">
    ...
    <button type="submit" class="finance-balance-overview-remove" title="{{ __('カードから外す') }}" aria-label="{{ $account['name'] }} {{ __('をカードから外す') }}">×</button>
  </form>
  ...
  <span class="finance-balance-overview-account-label">{{ $account['balanceLabel'] ?? __('残高') }}</span>
</article>
