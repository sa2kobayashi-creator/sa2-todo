      ...
      <div class="finance-report-toolbar no-print">
        <div class="finance-top-bar finance-report-top-bar">
          ...
          <div class="finance-report-actions">
            <a href="{{ $buildFinanceQuery($filters) }}" class="button-link secondary">{{ __('管理画面へ') }}</a>
            <button type="button" class="button-link" onclick="window.print()">{{ __('印刷') }}</button>
          </div>
        </div>
      </div>
      ...
      <article class="finance-report panel">
        ...
        <section class="finance-report-summary">
          ...
          <div class="finance-report-summary-grid">
            ...
            @if($filters['tab'] !== 'transfer')
              <div class="finance-report-stat">
                <span class="finance-report-stat-label">{{ __('振替出') }}</span>
                <span class="finance-report-stat-value">{{ $formatMoney($summary['transferOut'], $summary['currency']) }}</span>
              </div>
              <div class="finance-report-stat">
                <span class="finance-report-stat-label">{{ __('振替入') }}</span>
            ...
            <div class="finance-report-table-wrap">
              <table class="finance-report-table">
                <thead>
                  <tr>
                    <th>{{ __('口座') }}</th>
                    <th class="is-num">{{ __('収入') }}</th>
                    <th class="is-num">{{ __('支出') }}</th>
                    <th class="is-num">{{ __('収支') }}</th>
                  </tr>
                </thead>
                <tbody>
        ...
        @foreach([
          ...
          @if(!empty($groupedTransactions[$typeKey]))
            <section class="finance-report-section">
              <h2 class="finance-section-title">{{ $typeLabel }}{{ __('一覧') }}（{{ count($groupedTransactions[$typeKey]) }}{{ __('件') }}）</h2>
              <div class="finance-report-table-wrap">
                <table class="finance-report-table">
                  <thead>
                    <tr>
        ...
        @if($schedules !== [])
          <section class="finance-report-section">
            <h2 class="finance-section-title">{{ __('予定') }}（{{ count($schedules) }}{{ __('件') }}）</h2>
            <div class="finance-report-table-wrap">
              <table class="finance-report-table">
                <thead>
                  <tr>
        ...
        <section class="finance-report-section finance-report-balances">
          <h2 class="finance-section-title">{{ __('現在の口座残高（参考）') }}</h2>
          <div class="finance-report-table-wrap">
            <table class="finance-report-table">
              <thead>
                <tr>
                ...
                @foreach($accounts as $account)
                  <tr>
                    ...
                    <td class="is-num">
                      ...
                      @if(($account['balanceLabel'] ?? __('残高')) !== __('残高'))
          ...
          <div class="finance-report-balance-totals">
            @foreach($balanceTotals['totals'] as $currency => $total)
              <span>{{ $currency }} {{ __('総残高:') }} {{ $formatMoney($total, $currency) }}</span>
            @endforeach
            @foreach($balanceTotals['creditCards'] ?? [] as $currency => $amount)
              <span>{{ $currency }} {{ __('クレカ利用額:') }} {{ $formatMoney($amount, $currency) }}</span>
            @endforeach
          </div>
        </section>
      </article>
